...
class master(object):
  ...
  def read_parameters(self, parameters, callback=None):  
    ...
    # Register waiter and pending BEFORE sending to avoid race
    evt = threading.Event()
    with self._waiters_lock:
    ...
    # remember pending for fast match
      try:
        ...
        pass
      # Now write after waiter is armed
...
class _propar_builder(object):
  ...
  def read_pp_send_parameter_message(self, propar_message):
    ...
    # Trust only what we actually received
    message     = propar_message.get('data', []) or []
    declared    = int(propar_message.get('len', 0) or 0)
    message_len = min(declared, len(message))   # <-- clamp to real size
    ...
    while pos < message_len and read_status == PP_STATUS_OK:

      parameter = {}
      parameter['data'  ] = None
      parameter['action'] = 'write'

      if (parm_chained and (message_len - pos) < 1) or (not parm_chained and (message_len - pos) < 2):
        read_status = PP_STATUS_PROTOCOL_ERROR
      
      if read_status == PP_STATUS_OK:
        if not parm_chained:
          if pos >= message_len:
            read_status = PP_STATUS_PROTOCOL_ERROR
          else:
            proc_nr = message[pos]; pos += 1
        ...
        if parameter['parm_type'] == PP_TYPE_INT8:
            ...
            if (message_len - pos) < parameter['parm_size']: read_status = PP_ERROR_PROTOCOL_ERROR
            else:
              parameter['data'] = message[pos]; pos += 1
      ...
      if self.RECEIVE_START_1 is self.__receive_state:
        ...
        if received_byte == self.BYTE_DLE:
          ...
          if len(self.__receive_buffer) > 3:
            ...
            # Clamp 'len' to actual data length to avoid downstream overruns
