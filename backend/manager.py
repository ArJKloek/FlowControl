...
class ProparManager(QObject):
    ...
    def get_used_addresses_on_port(self, port: str, exclude_address: Optional[int] = None) -> set:
        """Return all known node addresses on a port, optionally excluding one address."""
        used = set()
        ex = None if exclude_address is None else int(exclude_address)
        for node in self._nodes:
            if node.port != port:
                continue
            addr = int(getattr(node, "address", 0))
            if ex is not None and addr == ex:
                continue
            used.add(addr)
        return used

    def change_primary_node_address(self, port: str, old_address: int, new_address: int) -> Tuple[bool, str]:
        """
        Write parameter 2 (Primary Node Address) and update local manager state.
        Returns (ok, message).
        """
        try:
            old_address = int(old_address)
            new_address = int(new_address)
        except Exception:
            return False, "Invalid node address"

        if new_address < 1 or new_address > 128:
            return False, "Node address must be between 1 and 128"

        if old_address == new_address:
            return True, "Node address unchanged"

        if new_address in self.get_used_addresses_on_port(port, exclude_address=old_address):
            return False, f"Node address {new_address} is already used on {port}"

        with self.port_lock(port):
            try:
                inst = self.get_shared_instrument(port, old_address)
            except Exception as e:
                return False, f"Could not connect to current node {old_address}: {e}"

            try:
                res = inst.writeParameter(2, int(new_address))
            except Exception as e:
            ...
            immediate_ok = (
                (res is True) or
                (res == 0) or
                (isinstance(res, dict) and res.get("status") == 0)
            )

            # Address switch can race ACK/readback; probe the new address briefly.
            if not immediate_ok:
                switched = False
                for _ in range(8):
                    try:
                        probe = self.get_shared_instrument(port, new_address)
                        rb = probe.readParameter(2)
                        if rb is not None and int(rb) == int(new_address):
                            switched = True
                            break
                    except Exception:
                        pass
                    time.sleep(0.1)
                if not switched:
                    return False, f"Node address write was not confirmed (result={res})"

            # Move any cached per-address state to the new address.
            old_key = (port, old_address)
            if old_key in self._gas_factors:
                self._gas_factors[(port, new_address)] = self._gas_factors.pop(old_key)

            for node in self._nodes:
                if node.port == port and int(getattr(node, "address", -1)) == old_address:
                    node.address = int(new_address)
                    break

            self.clear_shared_instrument_cache(port, old_address)

            if port in self._pollers:
                poller = self._pollers[port][1]
                old_period = float(poller._known.get(old_address, poller.default_period))
                poller.remove_node(old_address)
                poller.add_node(new_address, period=old_period)

            return True, f"Node address changed from {old_address} to {new_address}"
